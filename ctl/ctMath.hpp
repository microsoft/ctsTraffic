...
namespace ctl
{
    ...
    std::tuple<double, double> ctSampledStandardDeviation(const BidirectionalIterator& begin,
    ...
    ///
    template <typename BidirectionalIterator>
    std::tuple<double, double, double> ctInterquartileRange(const BidirectionalIterator& begin,
                                                            const BidirectionalIterator& end)
    {
        const auto size = end - begin;
        if (size < 3)
        ...
        const auto findMedian = [](const std::tuple<BidirectionalIterator, BidirectionalIterator>& split) -> double
        {
            ...
            FAIL_FAST_IF_MSG(
                rhs < lhs, "ctInterquartileRange internal error - the rhs iterator is less than the lhs iterator");
            ...
            default:
                FAIL_FAST_MSG("ctInterquartileRange internal error - returned iterators more than two apart [%Iu]",
                              rhs - lhs);
            }
