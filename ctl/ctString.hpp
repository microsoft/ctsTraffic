...
#pragma prefast(suppress:26018, "SAL doesn't offer a way to annotate the requirement that wcslen(convert_to_ptr(x)) == get_string_length(x)")
    ...
    //
    inline void replace_all(std::wstring& originalString,
    ...
    inline
    std::wstring replace_all_copy(std::wstring originalString,
                                  const std::wstring& searchString,
    ...
    inline void replace_all(std::string& originalString,
                            const std::string& searchString,
                            const std::string& replacementString) // NOLINT(google-runtime-references)
    {
        const auto searchSize = searchString.size();
        const auto replacementSize = replacementString.size();
        size_t index = 0;
    ...
    inline std::string replace_all_copy(std::string originalString,
                                        const std::string& searchString,
