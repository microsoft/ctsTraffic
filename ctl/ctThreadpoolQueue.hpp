...
namespace ctl
{
    ...
    public:
        ...
        ctThreadpoolQueueWaitableResultInterface(ctThreadpoolQueueWaitableResultInterface&&) = delete;
        ctThreadpoolQueueWaitableResultInterface(const ctThreadpoolQueueWaitableResultInterface&) = delete;

        ctThreadpoolQueueWaitableResultInterface&
        operator=(ctThreadpoolQueueWaitableResultInterface&&) = delete;
        ctThreadpoolQueueWaitableResultInterface&
        operator=(const ctThreadpoolQueueWaitableResultInterface&) = delete;
    ...
    private:
        struct TpEnvironment
        {
            ...
            using unique_tp_env = wil::unique_struct<
                TP_CALLBACK_ENVIRON, decltype(&DestroyThreadpoolEnvironment), DestroyThreadpoolEnvironment>;
            ...
            {
                wil::unique_threadpool_work newThreadpool(
                    CreateThreadpoolWork(callback, pv, m_threadPool ? &m_tpEnvironment : nullptr));
        ...
        static void CALLBACK WorkCallback(PTP_CALLBACK_INSTANCE, void* context, PTP_WORK) noexcept try
        {
            ...
            const auto resetThreadIdOnExit = wil::scope_exit([pThis]
            {
