...
namespace ctsUnitTest
{
    ...
    private:
        ...
        [[nodiscard]] std::unique_ptr<ctsIoPatternProtocolPolicy<ctsIoPatternProtocolTcpClient>>
        InitClientGracefulShutdownTest(uint64_t testTransferSize) const
        {
        ...
        [[nodiscard]] std::unique_ptr<ctsIoPatternProtocolPolicy<ctsIoPatternProtocolTcpServer>>
        InitServerGracefulShutdownTest(uint64_t testTransferSize) const
        {
            ctsConfig::g_configSettings->TcpShutdown = ctsConfig::TcpShutdownType::GracefulShutdown;
            g_isListening = true;
            g_transferSize = testTransferSize;
        ...
        [[nodiscard]] std::unique_ptr<ctsIoPatternProtocolPolicy<ctsIoPatternProtocolTcpClient>>
        InitClientHardShutdownTest(uint64_t testTransferSize) const
        {
        ...
        [[nodiscard]] std::unique_ptr<ctsIoPatternProtocolPolicy<ctsIoPatternProtocolUdp>>
        InitUdpClientTest(uint64_t testTransferSize) const
        {
            g_isListening = false;
            g_transferSize = testTransferSize;
        ...
        [[nodiscard]] std::unique_ptr<ctsIoPatternProtocolPolicy<ctsIoPatternProtocolUdp>>
        InitUdpServerTest(uint64_t testTransferSize) const
        {
            g_isListening = true;
            g_transferSize = testTransferSize;
        ...
        template <typename IoPattern>
        ctsTask RequestSendStatus(std::unique_ptr<ctsIoPatternProtocolPolicy<IoPattern>>& ioPattern,
                                  _In_ uint32_t* statusBuffer)
        {
            // GetNextPatternType
            auto task = ioPattern->GetNextPatternType();
        ...
        template <typename IoPattern>
        ctsTask RequestRecvStatus(std::unique_ptr<ctsIoPatternProtocolPolicy<IoPattern>>& ioPattern,
                                  _In_ uint32_t* statusBuffer)
        {
            // GetNextPatternType
            auto task = ioPattern->GetNextPatternType();
        ...
        template <typename IoPattern>
        void CompleteIoAndVerifySuccess(std::unique_ptr<ctsIoPatternProtocolPolicy<IoPattern>>& ioPattern,
                                        ctsTask task,
                                        uint32_t bytes)
        {
    ...
    public:
        ...
        TEST_METHOD(GracefulShutdownReceivedTooFewBytesForConnectionGuid)
        {
            ...
            Assert::AreEqual(ctsIoPatternError::NoConnectionGuid,
                             ctsIoPatternStateCheckProtocolError(testPattern->GetLastError()));
        ...
        TEST_METHOD(GracefulShutdownReceivedZeroBytesAfterConnectionGuid)
        {
            ...
            testPattern->CompletedTask(testTask, 0);
            Assert::AreEqual(ctsIoPatternError::ZeroByteXfer,
                             ctsIoPatternStateCheckProtocolError(testPattern->GetLastError()));
            Assert::IsTrue(testPattern->IsCompleted());
            VerifyNoMoreIo(testPattern);
        }
        ...
        TEST_METHOD(UDPReceivedZeroBytesAfterConnectionGuid)
        {
            ...
            const ctsTask testTask = RequestMoreIo(testPattern, 100);
            testPattern->CompletedTask(testTask, 0);
            Assert::AreEqual(ctsIoPatternError::NotProtocolError,
                             ctsIoPatternStateCheckProtocolError(testPattern->GetLastError()));
            Assert::IsFalse(testPattern->IsCompleted());
        }
        ...
        TEST_METHOD(GracefulShutdownClientFailTooManyBytes)
        {
            ...
            Assert::AreEqual(ctsIoPatternError::TooManyBytes,
                             ctsIoPatternStateCheckProtocolError(testPattern->GetLastError()));
