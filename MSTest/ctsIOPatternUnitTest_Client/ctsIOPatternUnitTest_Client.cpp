...
namespace ctsTraffic::ctsConfig
{
    ...
    float GetStatusTimeStamp() noexcept
    {
        return static_cast<float>(
            (ctl::ctTimer::snap_qpc_as_msec() - g_configSettings->StartTimeMilliseconds) / 1000.0);
    }
...
namespace ctsUnitTest
{
    ...
    public:
        ...
        TEST_METHOD(TestBaseClass_SuccessfulSend)
        {
            ...
            Assert::AreEqual(ctsIoStatus::ContinueIo,
                             test_pattern->CompleteIo(test_task, ctsStatistics::ConnectionIdLength, 0));
        ...
        TEST_METHOD(TestBaseClass_SuccessfulMultipleSends)
        {
            ...
            Assert::AreEqual(ctsIoStatus::ContinueIo,
                             test_pattern->CompleteIo(test_task1, ctsStatistics::ConnectionIdLength, 0));
        ...
        TEST_METHOD(TestClientBaseClass_TooManyBytesOnFINAfterSend)
        {
            ...
            Assert::AreEqual(static_cast<uint32_t>(c_statusErrorTooMuchDataTransferred),
                             test_pattern->GetLastPatternError());
        }
        ...
        TEST_METHOD(PushClient_NotVerifyingBuffersNotUsingSharedBuffer_Graceful)
        {
            ...
            for (uint32_t io_count = 0; io_count < 10; ++io_count)
            {
                ...
                Assert::AreEqual(ctsTaskAction::Send, test_task.m_ioAction);
                Logger::WriteMessage(
                    wil::str_printf<std::wstring>(L"%u: %ws", io_count, ToString<ctsTask>(test_task).c_str()).c_str());
                Assert::AreEqual(ctsIoStatus::ContinueIo,
                                 test_pattern->CompleteIo(test_task, g_TestRecvBufferLength, 0));
            }
        ...
        TEST_METHOD(PushClient_MultipleSendsWithISBEnabled)
        {
            ...
            for (uint32_t io_count = 0; io_count < 5; ++io_count)
            {
                ...
                Logger::WriteMessage(
                    wil::str_printf<std::wstring>(L"%u: %ws", io_count, ToString<ctsTask>(test_task_one).c_str()).
                    c_str());
                ...
                Logger::WriteMessage(
                    wil::str_printf<std::wstring>(L"%u: %ws", io_count, ToString<ctsTask>(test_task_two).c_str()).
                    c_str());
                ...
                Assert::AreEqual(ctsIoStatus::ContinueIo,
                                 test_pattern->CompleteIo(test_task_one, g_TestRecvBufferLength, 0));
                Assert::AreEqual(ctsIoStatus::ContinueIo,
                                 test_pattern->CompleteIo(test_task_two, g_TestRecvBufferLength, 0));
            }
        ...
        TEST_METHOD(PushClient_MultipleSendsWithISBEnabledInterleaving)
        {
            ...
            Assert::AreEqual(ctsTaskAction::Send, test_task.m_ioAction);
            Logger::WriteMessage(
                wil::str_printf<std::wstring>(L"%u: %ws", 0, ToString<ctsTask>(test_task).c_str()).c_str());
        ...
        TEST_METHOD(PushClient_LargeNumberOfSendsWithISBEnabled)
        {
            ...
            {
                Assert::AreEqual(ctsIoStatus::ContinueIo,
                                 test_pattern->CompleteIo(pended_tasks[ioCount], g_TestRecvBufferLength, 0));
        ...
        TEST_METHOD(PushClient_OneSendInFlightWithISBEnabledWhenISBIsSmallerThanBufferSize)
        {
            ...
            Assert::AreEqual(ctsIoStatus::ContinueIo,
                             testPattern->CompleteIo(testTask, ctsStatistics::ConnectionIdLength, 0));
            ...
            for (uint32_t ioCount = 0; ioCount < 10; ++ioCount)
            {
                ...
                Logger::WriteMessage(
                    wil::str_printf<std::wstring>(L"%u: %ws", ioCount,
                                                  ToString<ctsTask>(testTaskLoop).c_str()).c_str());
                ...
                Assert::AreEqual(ctsIoStatus::ContinueIo,
                                 testPattern->CompleteIo(testTaskLoop, g_TestRecvBufferLength, 0));
            }
        ...
        TEST_METHOD(PullClient_NotVerifyingBuffersNotUsingSharedBuffer_Graceful)
        {
            ...
            for (uint32_t io_count = 0; io_count < 10; ++io_count)
            {
                ...
                memcpy(test_task.m_buffer, ctsIoPattern::AccessSharedBuffer() + test_task.m_expectedPatternOffset,
                       test_task.m_bufferLength);
                Assert::AreEqual(ctsIoStatus::ContinueIo,
                                 test_pattern->CompleteIo(test_task, g_TestRecvBufferLength, 0));
            }
        ...
        TEST_METHOD(PullClient_NotVerifyingBuffersNotUsingSharedBuffer_SmallRecvs_Graceful)
        {
            ...
            Assert::AreEqual(ctsTaskAction::Recv, test_task.m_ioAction);
            Logger::WriteMessage(
                wil::str_printf<std::wstring>(L"%u: %ws", 10, ToString<ctsTask>(test_task).c_str()).c_str());
            // "recv" the correct bytes
            memcpy(test_task.m_buffer, ctsIoPattern::AccessSharedBuffer() + test_task.m_expectedPatternOffset,
                   test_task.m_bufferLength);
            Assert::AreEqual(ctsIoStatus::ContinueIo, test_pattern->CompleteIo(test_task, g_TestRecvBufferLength, 0));
