...
namespace ctsTraffic
{
    ...
    public:
        ...
        constexpr static const wchar_t* BuildProtocolErrorString(uint32_t status) noexcept
        {
            ...
            default:
                FAIL_FAST_MSG(
                    "ctsIOPattern: internal inconsistency - expecting a protocol error ctsIOProtocolState (%u)",
                    status);
            }
        }
        ...
        virtual void PrintTcpInfo(const socket_address& localAddr,
                                  const socket_address& remoteAddr,
                                  SOCKET socket) noexcept = 0;
        ...
        ctsIoStatus CompleteIo(const ctsTask& originalTask, uint32_t currentTransfer, uint32_t statusCode) noexcept;
    ...
    public:
        ...
        void PrintStatistics(const socket_address& localAddr, const socket_address& remoteAddr) noexcept override
        {
            ...
            {
                PRINT_DEBUG_INFO(
                    L"\t\tctsIOPattern::PrintStatistics :"
                    L" reporting a successful IO completion but transferred 0 bytes\n") ;
        ...
        void PrintTcpInfo(const socket_address& localAddr,
                          const socket_address& remoteAddr,
                          SOCKET socket) noexcept override
        {
