...
namespace ctsTraffic
{
    ctsMediaStreamServerListeningSocket::ctsMediaStreamServerListeningSocket(
        wil::unique_socket&& listeningSocket, const socket_address& listeningAddr) :
        m_threadIocp(
            std::make_shared<ctl::ctThreadIocp>(listeningSocket.get(), ctsConfig::g_configSettings->pTpEnvironment)),
    ...
    void ctsMediaStreamServerListeningSocket::RecvCompletion(OVERLAPPED* pOverlapped) noexcept
    {
        ...
        try
        {
            // scope to the object lock
            {
                ...
                {
                    m_priorFailureWasConectionReset = false;
                    const ctsMediaStreamMessage message(
                        ctsMediaStreamMessage::Extract(m_recvBuffer.data(), bytesReceived));
