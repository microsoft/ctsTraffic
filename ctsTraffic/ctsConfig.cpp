...
//
static void CheckReuseUnicastPort() noexcept
try
{
...
static std::wstring CheckOffloadRsc(const std::wstring& inputInterfaceDescription) noexcept
try
{
    const ctWmiService wmiService(L"ROOT\\StandardCimv2");
    ctWmiEnumerate wmiQuery(wmiService);
...
static std::wstring CheckOffloadLso(const std::wstring& inputInterfaceDescription) noexcept
try
{
    const ctWmiService wmiService(L"ROOT\\StandardCimv2");
    ctWmiEnumerate wmiQuery(wmiService);
...
static std::wstring CheckOffloadRss(const std::wstring& inputInterfaceDescription) noexcept
try
{
    const ctWmiService wmiService(L"ROOT\\StandardCimv2");
    ctWmiEnumerate wmiQuery(wmiService);
...
static std::wstring PrintPhysicalAdapter(const std::wstring& inputInterfaceDescription) noexcept
try
{
    const ctWmiService wmiService(L"ROOT\\StandardCimv2");
    ctWmiEnumerate wmiQuery(wmiService);
    ...
    for (const auto& setting : wmiQuery.query(L"SELECT * FROM MSFT_NetAdapterHardwareInfoSettingData"))
    {
        ...
        {
            locationInformationString.erase(
                locationInformationString.begin() +
                foundLocation,
                locationInformationString.end());
        }
        ...
        if (locationInformationString.starts_with(L"PCI"))
        {
            ...
            returnString = wil::str_printf<std::wstring>(
                L"Bus:%ws [PCI Link Speed %ws]",
                locationInformationString.c_str(),
                pciExpressCurrentLinkSpeed.c_str());
        }
        else
        {
...
static void ParseForConnect(vector<const wchar_t*>& args)
{
    ...
    const auto foundArg = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
...
static void ParseForAccept(vector<const wchar_t*>& args)
{
    ...
    const auto foundArgument = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
...
{
    auto foundArgument = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
    ...
    const auto foundPushbytes = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
    ...
    const auto foundPullbytes = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
    ...
    const auto foundBurstCount = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
    ...
    const auto foundBurstDelay = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
    ...
    if (!g_configSettings->TargetAddresses.empty())
    {
        ...
        if (0 == bindV4)
        {
            std::erase_if(
                ...
                [](const wil::network::socket_address& addr) noexcept
                {
...
{
    const auto foundRateLimit = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
    ...
    const auto foundRateLimitPeriod = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
...
{
    const auto foundVerbosity = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
    ...
    const auto foundStatusUpdate = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
    ...
    const auto foundConnectionFilename = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
    ...
    const auto foundErrorFilename = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
    ...
    const auto foundStatusFilename = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
    ...
    const auto foundJitterFilename = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
    ...
    const auto foundTcpInfoFilename = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
...
{
    const auto foundTimeLimitArgument = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
    ...
    const auto foundPauseAtEndArgument = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
...
{
    const auto foundCpuGroupIdArgument = ranges::find_if(
        args, [](const wchar_t* parameter) -> bool
        {
...
bool Startup(int argc, _In_reads_(argc) const wchar_t** argv)
{
    ...
    for (auto& addr : g_configSettings->ListenAddresses)
    {
        if (addr.port() == 0x0000)
        {
            addr.set_port(g_configSettings->Port);
        }
    }
    ...
    if (g_configSettings->LocalPortLow != 0)
    {
        const USHORT numberOfPorts =
            g_configSettings->LocalPortHigh == 0 ? 1 : static_cast<USHORT>(g_configSettings->LocalPortHigh - g_configSettings->LocalPortLow + 1);
...
void PrintNewConnection(
    ...
    const wil::network::socket_address& remoteAddr) noexcept
try
{
    ctsConfigInitOnce();
...
uint64_t GetTransferSize() noexcept
{
    ...
    return 0 == g_transferSizeHigh ? g_transferSizeLow : g_randomTwister.uniform_int(g_transferSizeLow, g_transferSizeHigh);
}
...
bool SetProcessDefaultCpuSets()
{
    ...
    if (!pfnSetProcessDefaultCpuSets || !pfnGetSystemCpuSetInformation)
    {
        const auto gle = GetLastError();
        PRINT_DEBUG_INFO(
            L"\t\tSetProcessDefaultCpuSets: GetProcAddress failed to load CpuSet functions: %lu\n",
            gle);
        return false;
    }
    ...
    if (returnedLength == 0)
    {
        ...
        const auto gle = GetLastError();
        PRINT_DEBUG_INFO(
            L"\t\tSetProcessDefaultCpuSets: GetSystemCpuSetInformation failed to get the length required: %lu\n",
            gle);
        return false;
    }

    const std::unique_ptr<BYTE[]> buffer(new BYTE[returnedLength]);
    if (!pfnGetSystemCpuSetInformation(
        reinterpret_cast<PSYSTEM_CPU_SET_INFORMATION>(buffer.get()), returnedLength,
    ...
    PRINT_DEBUG_INFO(
        L"\t\tSetProcessDefaultCpuSets: SetProcessDefaultCpuSets set process affinity to all CPU IDs in Group %u\n",
        settingsGroupId);
    return true;
}
...
#define SO_REUSE_UNICASTPORT (SO_PORT_SCALABILITY + 1)
        ...
        else if (localAddress.address_type() != NlatUnspecified && localAddress.port() == 0)
        {
            constexpr DWORD optValue{1}; // BOOL
            constexpr int optLength{sizeof optValue};
