...
namespace ctsTraffic::ctsConfig
{
    ...
    static std::wstring PrintPhysicalAdapter(const std::wstring& inputInterfaceDescription) noexcept try
    {
        ...
        for (const auto& setting : wmiQuery.query(L"SELECT * FROM MSFT_NetAdapterHardwareInfoSettingData"))
        {
            ...
            {
                locationInformationString.erase(locationInformationString.begin() + foundLocation,
                            ...
                            const socket_address& remoteAddr) noexcept try
    {
        ...
        if (writeToConsole)
        {
            ...
            const auto* const connectionString = ProtocolType::TCP == g_configSettings->Protocol
                                                     ? tcpConnectionString
                                                     : udpConnectionString;
                                ...
                                const ctsUdpStatistics& stats) noexcept try
    {
        ...
        const int64_t bitsPerSecond = elapsedTime > 0LL
                                          ? stats.m_bitsReceived.GetValueNoLock() * 1000LL / elapsedTime
                                          : 0LL;
