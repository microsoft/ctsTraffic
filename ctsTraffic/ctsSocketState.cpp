...
namespace ctsTraffic
{
	...
	void ctsSocketState::CompleteState(DWORD error) noexcept
	{
		...
		// notify the broker when initiating IO outside the internal lock
		if (initiating_io)
		{
			if (const auto parent = m_broker.lock())
			{
				parent->InitiatingIo();
			}
		}
	...
	VOID NTAPI ctsSocketState::ThreadPoolWorker(PTP_CALLBACK_INSTANCE, PVOID context, PTP_WORK) noexcept
	{
		...
		case InternalState::InitiatingIo:
		{
			try
			{
				...
				thisPtr->m_socket->CreateIoPattern();
		...
		// - this guarantees ctsSocket won't hold the final reference to the ctsSocketState
		...
		case InternalState::Closing:
		{
			...
			// don't hold the internal lock while closing the socket
			lock.reset();
			...
			lock = thisPtr->m_stateGuard.lock();
