...
namespace ctsTraffic
{
    ...
    ctsIoPatternError
    ctsIoPatternMediaStreamClient::CompleteTaskBackToPattern(const ctsTask& task, uint32_t completedBytes) noexcept
    {
        LARGE_INTEGER qpc;
        ...
        if (task.m_ioAction == ctsTaskAction::Recv)
        {
            ...
            if (!ctsMediaStreamMessage::ValidateBufferLengthFromTask(task, completedBytes))
            {
                ctsConfig::PrintErrorInfo(
                    L"ctsIoPatternMediaStreamClient received an invalid datagram trying to parse the protocol header");
                return ctsIoPatternError::TooFewBytes;
            }
            ...
            validationTask.m_bufferOffset = c_udpDatagramDataHeaderLength;
            // skip the UdpDatagramDataHeaderLength since we use them for our own stuff
            ...
            ctsConfig::g_configSettings->UdpStatusDetails.m_bitsReceived.Add(
                static_cast<int64_t>(completedBytes) * 8LL);
    ...
    vector<ctsConfig::JitterFrameEntry>::iterator
    ctsIoPatternMediaStreamClient::FindSequenceNumber(int64_t sequenceNumber) noexcept
    {
    ...
    void ctsIoPatternMediaStreamClient::SetNextStartTimer() const noexcept
    {
        if (m_startTimer != nullptr)
        {
            // convert to filetime from milliseconds
            // - make a 'relative' for SetThreadpoolTimer
            FILETIME relativeFileTime(
                ctTimer::convert_ms_to_relative_filetime(static_cast<int64_t>(m_frameRateMsPerFrame) + 500LL));
            // TP Timer APIs work off of the UTC time
    ...
    void ctsIoPatternMediaStreamClient::RenderFrame() noexcept
    {
        ...
        if (m_headEntry->m_receiverQpf != 0 && m_firstFrame.m_receiverQpf != 0)
        {
            ...
            const double msSinceFirstReceive =
                (static_cast<double>(m_headEntry->m_receiverQpc) * 1000.0f /
                    static_cast<double>(m_headEntry->m_receiverQpf)) -
                (static_cast<double>(m_firstFrame.m_receiverQpc) * 1000.0f /
                    static_cast<double>(m_firstFrame.m_receiverQpf));
            const double msSinceFirstSend =
                (static_cast<double>(m_headEntry->m_senderQpc) * 1000.0f /
                    static_cast<double>(m_headEntry->m_senderQpf)) -
                (static_cast<double>(m_firstFrame.m_senderQpc) * 1000.0f /
                    static_cast<double>(m_firstFrame.m_senderQpf));
    ...
    VOID
    CALLBACK
    ctsIoPatternMediaStreamClient::StartCallback(PTP_CALLBACK_INSTANCE, _In_ PVOID pContext, PTP_TIMER) noexcept
    {
        ...
        if (!thisPtr->ReceivedBufferedFrames())
        {
            ...
            resendTask.m_bufferType = ctsTask::BufferType::Static;
            // this is our own buffer: the base class should not mess with it
    ...
    VOID
    CALLBACK
    ctsIoPatternMediaStreamClient::TimerCallback(PTP_CALLBACK_INSTANCE, _In_ PVOID pContext, PTP_TIMER) noexcept
    {
        auto* thisPtr = static_cast<ctsIoPatternMediaStreamClient*>(pContext);
        ...
        while (!timerScheduled)
        {
            ...
            if (!fatalAborted)
            {
                ...
                else
                {
                    ...
                    thisPtr->SendTaskToCallback(abortTask);
                    PRINT_DEBUG_INFO(
                        L"\t\tctsIOPatternMediaStreamClient - issuing an ABORT to cleanly close the connection\n");
                }
            }
        }
    }
