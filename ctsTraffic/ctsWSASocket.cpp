...
void ctsWSASocket(const std::weak_ptr<ctsSocket>& weakSocket) noexcept // NOLINT(misc-use-internal-linkage)
{
    ...
    if (ctsConfig::g_configSettings->LocalPortHigh != 0 && ctsConfig::g_configSettings->LocalPortLow != 0)
    {
        ...
        nextPort =
            static_cast<uint16_t>(portCounter % (ctsConfig::g_configSettings->LocalPortHigh - ctsConfig::g_configSettings->LocalPortLow + 1)) +
