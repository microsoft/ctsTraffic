...
namespace ctsTraffic
{
    ...
    public:
        ...
        {
            return ctsConfig::StatusFormatting::Csv == format ? nullptr : FormatLegend(format);
        }
    ...
    protected:
        ...
        virtual PrintingStatus FormatData(const ctsConfig::StatusFormatting& format,
                                          int64_t currentTime,
                                          bool clearStatus) noexcept = 0;
        ...
        uint32_t AppendCsvOutput(uint32_t offset,
                                 uint32_t valueLength,
                                 _In_ PCWSTR value,
                                 bool addComma = true) noexcept
        {
            const auto converted = _snwprintf_s(
                m_outputBuffer + offset,
                c_outputBufferSize - offset,
    ...
    public:
        ...
        PrintingStatus FormatData(const ctsConfig::StatusFormatting& format,
                                  int64_t currentTime,
                                  bool clearStatus) noexcept override
        {
            const ctsUdpStatistics udpData(ctsConfig::g_configSettings->UdpStatusDetails.SnapView(clearStatus));
            const ctsConnectionStatistics connectionData(
                ctsConfig::g_configSettings->ConnectionStatusDetails.SnapView(clearStatus));
            ...
            if (ctsConfig::StatusFormatting::Csv == format)
            {
                ...
                charactersWritten += AppendCsvOutput(charactersWritten, c_timeSliceLength,
                ...
                charactersWritten += AppendCsvOutput(charactersWritten,
                                                     c_currentStreamsLength,
                                                     connectionData.m_activeConnectionCount.GetValueNoLock());
                charactersWritten += AppendCsvOutput(charactersWritten,
                                                     c_competedFramesLength,
                                                     udpData.m_successfulFrames.GetValueNoLock());
                charactersWritten += AppendCsvOutput(charactersWritten,
                                                     c_droppedFramesLength,
                                                     udpData.m_droppedFrames.GetValueNoLock());
                charactersWritten += AppendCsvOutput(charactersWritten,
                                                     c_duplicatedFramesLength,
                                                     udpData.m_duplicateFrames.GetValueNoLock());
                charactersWritten += AppendCsvOutput(charactersWritten,
                                                     c_errorFramesLength,
                                                     udpData.m_errorFrames.GetValueNoLock(),
            ...
            else
            {
                ...
                RightJustifyOutput(c_currentStreamsOffset,
                                   c_currentStreamsLength,
                                   connectionData.m_activeConnectionCount.GetValueNoLock());
                RightJustifyOutput(c_competedFramesOffset,
                                   c_competedFramesLength,
                                   udpData.m_successfulFrames.GetValueNoLock());
                RightJustifyOutput(c_droppedFramesOffset,
                                   c_droppedFramesLength,
                                   udpData.m_droppedFrames.GetValueNoLock());
                RightJustifyOutput(c_duplicatedFramesOffset,
                                   c_duplicatedFramesLength,
                                   udpData.m_duplicateFrames.GetValueNoLock());
                RightJustifyOutput(c_errorFramesOffset,
                                   c_errorFramesLength,
                                   udpData.m_errorFrames.GetValueNoLock());
    ...
    public:
                                  ...
                                  bool clearStatus) noexcept override
        {
            ...
            if (format == ctsConfig::StatusFormatting::Csv)
            {
                ...
                charactersWritten += AppendCsvOutput(charactersWritten,
                                                     c_currentTransactionsLength,
                                                     connectionData.m_activeConnectionCount.GetValueNoLock());
                charactersWritten += AppendCsvOutput(charactersWritten,
                                                     c_completedTransactionsLength,
                                                     connectionData.m_successfulCompletionCount.GetValueNoLock());
                charactersWritten += AppendCsvOutput(charactersWritten,
                                                     c_connectionErrorsLength,
                                                     connectionData.m_connectionErrorCount.GetValueNoLock());
                charactersWritten += AppendCsvOutput(charactersWritten,
                                                     c_protocolErrorsLength,
                                                     connectionData.m_protocolErrorCount.GetValueNoLock(),
                                                     false); // no comma at the end
                TerminateFileString(charactersWritten);
            }
            else
            {
                ...
                RightJustifyOutput(c_currentTransactionsOffset,
                                   c_currentTransactionsLength,
                                   connectionData.m_activeConnectionCount.GetValueNoLock());
                RightJustifyOutput(c_completedTransactionsOffset,
                                   c_completedTransactionsLength,
                                   connectionData.m_successfulCompletionCount.GetValueNoLock());
                RightJustifyOutput(c_connectionErrorsOffset,
                                   c_connectionErrorsLength,
                                   connectionData.m_connectionErrorCount.GetValueNoLock());
                RightJustifyOutput(c_protocolErrorsOffset,
                                   c_protocolErrorsLength,
                                   connectionData.m_protocolErrorCount.GetValueNoLock());
                if (format == ctsConfig::StatusFormatting::ConsoleOutput)
                {
