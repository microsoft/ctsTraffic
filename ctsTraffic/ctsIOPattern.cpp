...
static BOOL CALLBACK InitOnceIoPatternCallback(PINIT_ONCE, PVOID, PVOID*) noexcept // NOLINT(bugprone-exception-escape)
{
    ...
    if (WI_IsFlagClear(ctsConfig::g_configSettings->SocketFlags, WSA_FLAG_REGISTERED_IO))
    {
        DWORD oldSetting{};
        FAIL_FAST_IF_MSG(
            !VirtualProtect(g_senderSharedBuffer, g_maximumBufferSize, PAGE_READONLY, &oldSetting), "VirtualProtect failed: %lu", GetLastError());
    }
...
void ctsIoPattern::CreateRecvBuffers()
{
    ...
    if (recvCount > 0)
    {
        ...
        if (ctsConfig::g_configSettings->UseSharedBuffer)
        {
            for (auto bufferCount = 0ul; bufferCount < recvCount; ++bufferCount)
            {
                ...
                {
                    m_receivingRioBufferIds[bufferCount].m_bufferId =
                        ctsConfig::g_configSettings->RioFunctions.f.RIORegisterBuffer(
    ...
    if (WI_IsFlagSet(ctsConfig::g_configSettings->SocketFlags, WSA_FLAG_REGISTERED_IO))
    {
        ...
        m_rioCompletionMessage.m_bufferId =
            ctsConfig::g_configSettings->RioFunctions.f.RIORegisterBuffer(
                m_completionMessageBuffer.data(),
                static_cast<DWORD>(m_completionMessageBuffer.size()));
...
void ctsIoPattern::CreateSendBuffers()
{
    ...
    if (WI_IsFlagSet(ctsConfig::g_configSettings->SocketFlags, WSA_FLAG_REGISTERED_IO))
    {
        ...
        {
            sendingBuffer.m_bufferId = ctsConfig::g_configSettings->RioFunctions.f.RIORegisterBuffer(g_senderSharedBuffer, g_maximumBufferSize);
...
// requires that the caller has locked the socket
//
ctsIoStatus ctsIoPattern::CompleteIo(const ctsTask& originalTask, uint32_t currentTransfer, uint32_t statusCode) noexcept
        ...
        if (m_bytesSendingPerQuantum > 0)
        {
            ...
            if (m_bytesSendingThisQuantum < m_bytesSendingPerQuantum)
            {
                ...
                if (currentTimeMs > (m_quantumStartTimeMs + ctsConfig::g_configSettings->TcpBytesPerSecondPeriod))
                {
                    ...
                    const auto quantumsSkippedSinceLastSend =
                        (currentTimeMs - m_quantumStartTimeMs) / ctsConfig::g_configSettings->TcpBytesPerSecondPeriod;
...
{
    PRINT_DEBUG_INFO(
        L"\t\tctsIOPatternMediaStreamServer - frame rate in milliseconds per frame : %lld\n", static_cast<int64_t>(1000UL / m_frameRateFps));
}
